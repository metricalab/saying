...
public class PersistentContext {
	...
	public InMemorySaying buildSaying() {
		...
		listsaying.add(new SayingDTO(Long.valueOf(1), "A caballo regalado no le mires el diente", 3,
				ConstantsData.SPRING_BOOT_MEMORY));
		listsaying.add(new SayingDTO(Long.valueOf(2), "A falta de pan, buenas son tortas", 4,
				ConstantsData.SPRING_BOOT_MEMORY));
		listsaying.add(new SayingDTO(Long.valueOf(3), "A la arrogancia en el pedir, la virtud del no dar", 12,
				ConstantsData.SPRING_BOOT_MEMORY));
		listsaying
				.add(new SayingDTO(Long.valueOf(4), "A la tercera va la vencida", 3, ConstantsData.SPRING_BOOT_MEMORY));
		listsaying.add(
				new SayingDTO(Long.valueOf(5), "A palabras necias, oídos sordos", 5, ConstantsData.SPRING_BOOT_MEMORY));
		listsaying.add(
				new SayingDTO(Long.valueOf(6), "Al pan, pan, y al vino, vino", 11, ConstantsData.SPRING_BOOT_MEMORY));
		listsaying.add(new SayingDTO(Long.valueOf(7), "Al perro flaco, todo se le vuelven pulgas", 1,
				ConstantsData.SPRING_BOOT_MEMORY));
		listsaying.add(new SayingDTO(Long.valueOf(8), "Al que Dios se la dé, San Pedro se la bendiga", 9,
				ConstantsData.SPRING_BOOT_MEMORY));
		listsaying.add(new SayingDTO(Long.valueOf(9), "Antes se coge al mentiroso que al cojo", 8,
				ConstantsData.SPRING_BOOT_MEMORY));
		listsaying.add(
				new SayingDTO(Long.valueOf(10), "Burro grande, ande o no ande", 2, ConstantsData.SPRING_BOOT_MEMORY));
